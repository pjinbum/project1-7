...
const Corona = () => {
  ...
  useEffect(() => {
    ...
    const fetchCorona = async () => {
      const response = await fetch("http://apis.data.go.kr/1790387/covid19CurrentStatusKorea/covid19CurrentStatusKoreaJason?serviceKey=69MDcrrkbf0Z3ZW4LFyqSZ%2BLToQPK3vjVrDrzz3qtpGyGG2JezYDMin%2FLsyv3NWBK%2BejgMVU4p879wUSlznU1Q%3D%3D");
    ...
    const fetchDJCorona = async () => {
      const response = await fetch(`http://apis.data.go.kr/1352000/ODMS_COVID_04/callCovid04Api?serviceKey=69MDcrrkbf0Z3ZW4LFyqSZ%2BLToQPK3vjVrDrzz3qtpGyGG2JezYDMin%2FLsyv3NWBK%2BejgMVU4p879wUSlznU1Q%3D%3D&std_day=2023-${currentMonth}-${yesterday}&gubun=%EB%8C%80%EC%A0%84&apiType=JSON`);
  ...
  const props1 = useSpring({ from: { number: 0 }, to: { number: coronaConfirm }, config: { tension: 40 }, });

  const props2 = useSpring({ from: { number: 0 }, to: { number: coronaDeath }, config: { tension: 17 }, });

  const props3 = useSpring({ from: { number: 0 }, to: { number: djConfirm }, config: { tension: 500 }, });
  ...
  return (
    <div>
      ...
      <div className='Covid-timeNow'>{currentyear}. {currentMonth}. {today}. 0시 기준</div>
